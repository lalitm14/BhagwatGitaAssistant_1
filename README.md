...
- **Talking Avatar**: Generates a video of a talking head (SadTalker) synchronised with the audio **(!!) However please take note in the curent version of this app released here SadTalker talking-head fature has not been implemented**.
...
- **Git** installed (to clone the repository). Download using browser from : https://github.com/git-for-windows/git/releases/download/v2.54.0.windows.1/Git-2.54.0-64-bit.exe
- **Huggingface-CLI** download using Windows command prompt from : huggingface-cli - powershell -ExecutionPolicy ByPass -c "irm https://hf.co/cli/install.ps1 | iex"
